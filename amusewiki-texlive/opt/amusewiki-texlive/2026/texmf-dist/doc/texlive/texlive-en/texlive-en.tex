% $Id: texlive-en.tex 78419 2026-03-18 11:19:15Z mseven $
...
\title{%
  {\huge \textit{The \TeX\ Live Guide---2026}}
}
...
\subsection{\protect\TeX\protect\ Live}

This document describes the main features of the \TL{} software
distribution\Dash \TeX{} and related programs for \GNU/Linux and other
Unix flavors, \macOS, and Windows systems. You may have acquired \TL{}
by downloading, or on a \DVD, or in some other way.
...
As to Windows: Windows~10 and later are supported. Windows 7 and Vista
...
You can install \TL{} either from \DVD{} (or an \ISO\ image) or over the
Internet (\url{https://tug.org/texlive/acquire.html}). The net installer
itself is small, and downloads everything requested from the Internet.

You can prepare a runnable installation on, e.g., a \USB{} stick (see
section~\ref{sec:portable-tl}). Installation is described in detail in
later sections (\p.\pageref{sec:install}), but here is a quick start:
...
their web interfaces, to \email{texhax@tug.org} through email (it's not
...
\subsection{The predefined texmf trees}
...
\item [TEXMFCONFIG] The (personal) tree used by the utilities
  \verb+texconfig+, \verb+updmap-user+, and \verb+fmtutil-user+ to store modified
  configuration data.
...
\begin{description}
  ...
  \begin{ttdescription}
    \item[2025] A previous release.
    \item[2026] The current release.
    \begin{ttdescription}
      \item [bin] ~
      \begin{ttdescription}
  ...
  \item[user's home directory] (\texttt{\$HOME} or
      \texttt{\%USERPROFILE\%})
    \begin{ttdescription}
      \item[.texlive2025] Privately generated and configuration data
        for a previous release.
      \item[.texlive2026] Privately generated and configuration data
...
\TeX\ (also known as \TeX\ engines), all derived from the original program.
...
TrueType- and system fonts. It also incorporates MetaPost as a library
for fast generation of graphics. Most of all, it includes a Lua interpreter
...
Lua interpreter. Its web site is \url{https://www.luatex.org/}, and the
reference manual is \OnCD{texmf-dist/doc/luatex/base/luatex.pdf}.

\item [LuaMeta\TeX] is a descendant of Lua\TeX, and is the default
engine for \ConTeXt. Most features are implemented in Lua to enable
maximum flexibility, including all font handling and backend output.
See \url{https://ctan.org/pkg/context}; the reference manual is
\OnCD{texmf-dist/doc/luametatex/base/luametatex.pdf}.
...
\item [Hi\TeX] outputs its own HINT format, designed for mobile devices.
See \url{https://ctan.org/pkg/hitex}.

\item [(e)(u)p\TeX] provide native support for Japanese typesetting
requirements; p\TeX\ is the basic engine, while the `e-' variants add
\eTeX\ functionality and `u-' add Unicode support.

\item [\OMEGA\ (Omega)] is the original engine supporting Unicode,
written in the 1990s. supports working with almost all the world's
scripts simultaneously. It also supports so-called `\OMEGA{} Translation
Processes' (OTPs), for performing complex transformations on arbitrary
input. Omega is no longer included in \TL{} as a separate program; only
Aleph is provided:
...
For more information about the capabilities and histories of the
different engines and the formats built with them, see the article
``Overview of \TeX\ engines and formats'',
\url{https://tug.org/TUGboat/tb46-3/tb144berry-engines-formats.pdf}.
For a brief comparison of engines vs.\ formats vs.\ packages, see the
web page \url{https://tug.org/levels}.
...
To begin, the most common method is to download the \TL{} net installer.
See \url{https://tug.org/texlive/acquire.html} for more information and
...
\filename{install-tl-windows.bat} will be in the
subdirectory \dirname{install-tl-yyyymmdd}, where yyyymmdd
represents the date when the installer was generated.
...
\item[\DVD\ or \ISO\ image] The unpacking will already be done, so you just
run \filename{install-tl} as described below.
...
A separate distribution is prepared for \macOS, named Mac\TeX\
(\url{https://tug.org/mactex}). We recommend using the native Mac\TeX\
installer instead of the \TL\ installer on \macOS, because the native
installer makes a few Mac-specific adjustments, in particular to allow
easily switching between different \TL\ releases on Macs, using the
so-called \TeX{}Dist data structure.

Mac\TeX\ is a superset of \TL, and the main \TeX\ trees and binaries
...
\begin{alltt}
> \Ucom{C:\bs{}tltemp\bs{}install-tl-windows}
\end{alltt}
Figure~\ref{fig:basic-w32} displays the initial basic screen of the
\GUI\ installer, which is the default for Windows.
...
users are forced to create a Microsoft network account upon
...
you choose a ``scheme'', which is an overall set of packages and collections.
...
\dirname{/usr/local/texlive/2026} on Unix and
|C:\texlive\2026| on Windows.  This arrangement enables
...
create |C:\texlive\2026| (or, more generally,
|%SystemDrive%\texlive\2026|).
...
But do not use your home directory on Windows if the name of your home
or `userprofile' contains non-ASCII characters. We are looking into
possible solutions, but cannot promise anything at this point. See also
\url{https://tug.org/texlive/windows.html\#nonascii}.
...
\subsubsection{Internet updates after \DVD\ or \ISO\ installation}
\label{sec:dvd-install-net-updates}

If you installed \TL\ from \DVD\ (see \url{https://tug.org/dvd}) or an
\ISO\ image and then wish to get updates from the Internet, you need to
run this command---\emph{after} you've updated your search path (as
...
\item[PostScript support.] For PostScript files, a PSviewer filetype
  ...
  displayed by the default PDF viewer.
...
new \TL{} would never get precedence.  As a workaround, the installer
...
\file{.../2026/texmf.cnf}.  If you must make changes to the defaults
(not normally necessary), this is the place to put them.  The main
configuration file is in \file{.../2026/texmf-dist/web2c/texmf.cnf}.
...
\item Akira Kakuto, for providing the Windows binaries and many other
...
\item Tim Stewart, for creating the torrents.
...
Wang Ran, S YS, Jjgod Jiang, Jinsong Zhao, Yue Wang, \& Helin Gai (Chinese),
...
Adelheid Grob (German).  The \TL{} documentation web page
...
single \CD, so we split it into three different distributions.
In addition:
...
as discussed in a \textsl{TUGboat} article by Hironori Kitagawa
...
\TL. See the separate \TL{} Windows web page
...
\item Support the \code{named} keyword in the \code{pdf:image} special.
\end{itemize*}
...
\noindent Mac\TeX: contains the latest Ghostscript 10.03.0, updates
...
\begin{itemize*}
\item Generate PDF-1.7 in all formats, both plain and \LaTeX{}; before, it was
  1.5. You may need an older version, especially for printing companies.
  You can generate, e.g., PDF-1.4 in various ways:
  \begin{itemize}
  \item Running \texttt{ps2pdf14} on the PDF file after it's generated
       by \TeX{}; this simple wrapper is part of Ghostscript. Other
       external software can do the same job.
  \item \LaTeX{} with PDF backends: \verb|\pdf_version_gset:n{1.4}|
  \item pdf\TeX{}: \verb|\pdfmajorversion=1 \pdfminorversion=4|
  \item Lua\TeX{}:
          \verb|\pdfvariable majorversion=1 \pdfvariable minorversion=4|
  \item \XeTeX{}:
          \verb|\special{pdf:majorversion:1}\special{pdf:minorversion:4}|
  \end{itemize}
\item If \TeX{} exits due to the \texttt{-halt-on-error} option,
  write the help message to the log file, as happens by default.
\item New primitive parameter \cs{ignoreprimitiveerror}; if set to 1,
  the error \texttt{"Infinite glue shrinkage found in box being
  split"} becomes a warning, so the program exit status is not changed
  to failure.
  Other values are reserved for future use. (Background:
  \url{https://www.latex-project.org/news/latex2e-news/ltnews39.pdf},
  section ``Enhancements to the new mark mechanism''.
\item Scaling fonts to $\ge2048$pt now results in an error message, instead
  of (unhandled) arithmetic overflow or silent changing of the user's value.
\end{itemize*}

\noindent Lua\TeX: \begin{itemize*}
\item LuaJIT 2.1.1736781742.
\item New \texttt{luaffi} lib, supporting aarch64.
\item If \cs{tracinglostchars=4}, warn about missing characters when
  glyphs are processed, instead of waiting until typesetting needs
  them. If \cs{tracinglostchars=5}, make that ``early'' diagnostic
  into an error.
\item New primitive \cs{mathemptydisplaymode}; when set
  to 1, empty paragraphs before a display equation will be ignored. By
  default paragraphs before a display equation containing dir nodes
  are not ignored, which has been the traditional behavior.
\item For PNG handling, we no longer set the attr on the mask. If we get
  a report about dropping this we could add a \texttt{maskattr}
  feature, but one should consider to include a correct pdf of the
  image instead, as it's more robust.
\item Handle token combiners in immediate assignments.
\item Cross-engine updates noted above.
\end{itemize*}
...
\noindent pdf\TeX: \begin{itemize*}
\item New primitive \cs{pdfptexuseunderscore} to use \verb|_| instead of
  \verb|.|\ in various dictionary entries, e.g., \verb|PTEX_Fullbanner|
  instead of \verb|PTEX.Fullbanner|.
\item Don't omit real spaces just because the font changes.
\item Increase tolerance for detecting interword spaces in overfull lines.
\item Ignore tiny resolutions like 1x1 embedded in images instead of
  exceeding \cs{maxdimen}.
\item Cross-engine updates noted above.
\end{itemize*}

\noindent ep\TeX{}, eup\TeX{}: \begin{itemize*}
\item Support combining characters with multiple codepoints,
  Unicode Latin characters and 16-bit OFM Level-0 with
  new kcatcodes: \texttt{modifier}(20), \verb|latin_ucs|(14).
\item Cross-engine updates noted above.
\end{itemize*}
...
\noindent Dvipdfmx: \begin{itemize*}
\item The \texttt{ebb} executable is now a separately-maintained script
  instead of a link to \texttt{dvipdfmx}.
\item Support up\TeX{} new encoding for combining characters via virtual fonts.
\item Fixes for embedded TrueType fonts with $>127$ glyphs, and more.
\item Fixes for including images multiple times.
\end{itemize*}

\noindent CWEB: \begin{itemize*}
\item New option \texttt{+F} for cweave and ctwill for compound statements.
\item Sync pdf bookmark pages with table-of-contents.
\item Integration of Martin Ruckert's solution for faster index sorting.
\item Support macro \cs{9} for sorting names-of-the-sections.
\item Improved sorting algorithm in \texttt{ctwill-proofsort}.
\item Output names-of-the-sections in \texttt{ctwill} format.
\end{itemize*}

Dvips: \begin{itemize*}
\item Support up\TeX{} new encoding for combining characters via
    virtual fonts.
\end{itemize*}

\noindent Kpathsea: \begin{itemize*}
\item \texttt{kpsewhich} outputs a blank line when a given file cannot be
    found, if more than one file to search for is specified.  
\end{itemize*}

\noindent \code{tlmgr}: \begin{itemize*}
\item \texttt{tlmgr info}: new option \texttt{--only-files} to just
    report the files for a given package.
\item \texttt{tlmgr search}: new option \texttt{--json}.
\end{itemize*}

\noindent Platforms: \begin{itemize*}
\item The \verb|x86_64-linux| binaries are now built on Alma Linux~8,
...
older systems will be available
(see \url{https://tug.org/texlive/custom-bin.html}).

\item \TL\ now provides a file \texttt{scripts-targets.lst} with both the name
and the target for symlinked scripts, for the convenience of distros.
It's in the source directory
\texttt{Build/source/texk/texlive/linked\_scripts}. The longstanding
file \texttt{scripts.lst}, with just the targets, in that directory, is
still maintained.
\end{itemize*}

\noindent Windows:
\begin{itemize*}
\item Visual Basic usage in \texttt{uninstall-windows} and
\texttt{runscript.tlu} has been replaced by Powershell, since VB is
optional as of Windows 11.
\end{itemize*}

\noindent Mac\TeX: \hangindent=\leftmargin
Again this year, Mac\TeX{}-2025 supports both Arm and Intel processors
on macOS 10.14 (Mojave, 2018), and higher. It installs:
\begin{itemize*}
\item \TeX{} Live 2025.
\item Ghostscript-10.04.0.
\item \texttt{libgs} and \texttt{mutool}, for \texttt{dvisvgm}. In
  previous years, this \texttt{dvisvgm} support required X11, but that
  requirement is eliminated in 2025 and \texttt{dvisvgm} will work both
  with and without X11.
\item HintView 2.0, which understands the color extensions added to the
  latest \texttt{hitex} in \TeX{} Live 2025.
\item Asymptote, which now requires macOS 11 (Big Sur, 2020) or higher.
\item TeXShop 5.49, which supports Apple's latest Apple Intelligence
  features, including Writing Tools.
\end{itemize*}

% 
\htmlanchor{news}
\subsection{Present: 2026}
\label{sec:tlcurrent}

Cross-engine changes (except in original \TeX{} and Aleph):
\begin{itemize*}
\item When the ``\texttt{Infinite glue shrinkage}'' error is
ignored (see the 2025 release notes), the word ``error'' is no longer
included in the output, and the message is written only to the log file.
This was a source of minor confusion and false positives.
\end{itemize*}

\noindent Hi\TeX: \begin{itemize*}\raggedright
\item Now exclusively uses the UTF-8 input encoding.
\item OpenType and TrueType fonts are now supported minimally (no
ligatures, no math); this will improve in time.
\item \cs{Uchar} and \cs{Ucharcat} primitives added.
\item The \LaTeX{} kernel needs updates for this version, so \texttt{hilatex}
is not available for the time being.
\item Major changes throughout the code base, including the HINT viewers, to
implement the above. Use with care!
\item Cross-engine updates noted above.
\end{itemize*}

\noindent Lua\TeX: \begin{itemize*}
\item New \texttt{luaffi}.
\item Lua bindings for the HarfBuzz subset API.
\item \texttt{read\_img} now looks in a given \texttt{--output-directory}.
\item Saves and restores \texttt{pdf->objcompresslevel} along with
      \texttt{pdf->compress\_level}.
\item \texttt{texlua} errors print tracebacks.
\item \texttt{set\_macro} check for removing trailing space improved.
\item Wrong spacing around display with \cs{matheqdirmode=1}
      partially fixed.
\item Restored \cs{mathflattenmode}.
\item Fixes for legacy handling of TrueType fonts.
\item Experimental extension to \texttt{ToUnicode} for Type~1 fonts.
\item Maximum size of the destination names table for PDF output raised to
      1048576.
\item Additional \texttt{value} field for write nodes (\texttt{getfield}) for
      unexpanded token tables.
\item Some unused modes made obsolete.
\item Cross-engine updates noted above.
\end{itemize*}

\noindent MetaPost: \begin{itemize*}\raggedright
\item \texttt{mp\_number\_to\_scaled} returns a 64-bit signed integer instead
of the system-dependent \texttt{int}. Internally, start on replacing
\texttt{int} with \texttt{int64\_t} in general.
\item \texttt{mp\_hard\_times} fixed.
\item Fixes for \texttt{weave}.
\item Help message reorganized and updated for consistency.
\end{itemize*}

\noindent pdf\TeX: \begin{itemize*}
\item Don't complain about PDF minor version $\le2$ unless major version $=1$.
\item Don't erroneously shift fraction denominators or other text if
      \cs{pdfpageheight} is used.
\item Fix overlapping text in \texttt{ptmr8r} and other fonts.
\item If the environment variable \code{TEXMF\_DEBUG\_PNG\_COPY} is set to~1,
      report attributes of each PNG image and whether it was copied.
\item Cross-engine updates noted above.
\end{itemize*}
...
\noindent Dvipdfmx: \begin{itemize*}
\item Option \texttt{-o -} to send output to stdout reenabled, after being
unintentionally disabled for several years.
\end{itemize*}

\noindent CWEB: \begin{itemize*}
\item All CWEB programs work \texttt{--verbose}ly by default again.
\item All CWEB programs no longer fail in case of warning messages.
\item \texttt{ctangle} produces far less whitespace in preprocessor macros.
\item \texttt{ctwill-proofsort} supports multiple definitions for the same
      identifier.
\end{itemize*}

%\noindent Dvips: \begin{itemize*}
%\end{itemize*}

\noindent Upmendex: \begin{itemize*}
\item Experimental support for Brahmic scripts (Bengali, Gurmukhi,
Gujarati, Kannada, Lao, Malayalam, Oriya, Sinhala, Tamil, Telugu).
\end{itemize*}

\noindent Kpathsea: \begin{itemize*}\raggedright
\item New configuration value \texttt{texmf\_nlink\_for\_leaf} to specify
whether the \texttt{st\_nlink=2} check is made to decide whether
we're in a leaf directory; in the default \texttt{texmf.cnf}, the check is
now disabled.

\item The configuration value \texttt{openin\_any} no longer has any effect;
...
knowledge, no value but `\texttt{a}' was ever useful (or used).
\end{itemize*}

\noindent \code{tlmgr}: \begin{itemize*}
\item Check for backup container creation failure.
\item In \texttt{--usermode}, do not warn about the
      \texttt{texlive-scripts} package being missing.
\end{itemize*}

\noindent Other programs: \begin{itemize*}
\item \texttt{fmtutil, updmap}: make self-locating as with \texttt{tlmgr}.
\item New program \texttt{xdvipsk}, an extended \texttt{dvips} with
      support for more image formats and (with Lua\TeX{}) OpenType and
      TrueType fonts. See the article by Sigitas Tolusis in
      \textsl{TUGboat}, \url{https://tug.org/TUGboat/tb38-2/tb119tolusis.pdf}.
\item Many man pages substantially updated.
\end{itemize*}

\noindent Platforms: \begin{itemize*}
\item See Mac\TeX{} note just below.
\item The \texttt{i386-linux}, \texttt{aarch64-linux}, and
      \texttt{armhf-linux} binaries are now built on Debian bullseye
      (initially released 2021).
\end{itemize*}

\noindent Mac\TeX: \hangindent=\leftmargin
Mac\TeX{}-2026 now supports Big Sur (2020, macOS~11) and higher, on both
Intel and Arm. Support for Mojave (2018) and Catalina (2019,
macOS~10) has been dropped. It installs:
\begin{itemize*}
\item \TeX{} Live 2026;
\item Ghostscript-10.06.0;
\item \TeX{}Shop, including a special version supporting Liquid Glass;
\item \LaTeX{}iT, BibDesk, HintView.
\end{itemize*}

The companion small Basic\TeX{} distribution for Macs (download size
about 120MB, \url{https://tug.org/mactex/morepackages.html}) now
includes all core packages for tagging support, the \LaTeX{}
\texttt{-dev} engines for testing, and \texttt{tex4ht}. Those who would
like to install this on other platforms can use \texttt{scheme-small}.
Basic\TeX{} supports all three main typesetting engines: pdf\LaTeX{},
Xe\LaTeX{}, and Lua\LaTeX{}.

\smallskip

\noindent Finally: many thanks to Siep Kroonenberg for her work on
\TeX{} Live, especially Windows support, for many years. She has retired
and is moving on to other projects. Thanks again Siep, and best wishes!
...
\begin{quote}
\email{tex-live@tug.org} {\small (public mailing list,
                          you need not subscribe in order to post)}\\
\url{https://tug.org/texlive}
\end{quote}
